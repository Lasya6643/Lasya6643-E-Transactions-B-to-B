...
<html>
<head>
<title> Tenderslist Modify Form</title>
</head>
...
<input name=dot type=text disabled size=10  VALUE=<%=ds%> readonly></td></tr>
...
<input name=bsd onfocus='dd();' type=text value=<%=ds1%> disabled size=10 ><input type=button value=calendar onclick='dd();'></td></tr>

<tr><td class=td><b>Bid Submission Closing Date</b></td><td>
<input name=bsscd type=text disabled value=<%=ds2%> onfocus='dd1();' size=10  > <input type=button value=calendar onclick='dd1();'></td></tr>
<tr><td class=td><b>Date of Notice</b></td><td>
<input name=don type=text disabled size=10  value=<%=ds3%> onfocus='dd2();'> <input type=button value=calendar onclick='dd2();'></td></tr>

<tr><td class=td><b>Tender Fee</b></td><td>
<input name=tenderfee type=text disabled size=10 value=<%=rs1.getInt(8)%> onkeypress='if(event.keyCode<48 || event.keyCode>58){tenderslist.tenderfee.focus(); return false;}if(tenderslist.tenderfee.value.length>10)return false;'></td></tr>
...
<textarea rows=5 cols=30 name=tenderwork disabled size=100><%=rs1.getString(11)%> </textarea>
</td></tr>
<tr><td class=td><b>Tender Amt</b></td><td>
<input name=ecost type=text disabled size=10 value=<%=rs1.getInt(12)%>  onkeypress='if(event.keyCode<48 || event.keyCode>58){tenderslist.ecost.focus(); return false;}if(tenderslist.ecost.value.length>10)return false;'></td></tr>

<tr><td class=td><b>Period of Completion</b></td><td>
<input name=poc type=text disabled size=2 value=<%=rs1.getInt(13)%>  onkeypress='if(event.keyCode<48 || event.keyCode>58){tenderslist.poc.focus(); return false;}if(tenderslist.poc.value.length>2)return false;'></td></tr>
...
<input name=bcall type=text disabled size=2 value=<%=rs1.getString(15)%>  onkeypress='if(event.keyCode<48 || event.keyCode>58){tenderslist.bcall.focus(); return false;}if(tenderslist.bcall.value.length>2)return false;'></td></tr>
...
<input name=doa type=text disabled size=10  value=<%=ds4%>  onfocus='dd3();'><input type=button value=calendar onclick='dd3();'></td></tr>
...
<input name=paymentperentage type=text value=<%=rs1.getInt(19)%> disabled size=10 onkeypress='if(event.keyCode<48 || event.keyCode>58){tenderslist.paymentperentage.focus(); return false;}if(tenderslist.paymentperentage.value.length>10)return false;'></td></tr>
...
<center>
<input type="button" value="Update"  class=border onclick=enableAll();>
